...
@task
def build_listing_providerguide(listing):
        """
    Scrapes and completes a provider guide listing. Then updates the api with the data
    """

    soup = BeautifulSoup(urllib.urlopen(listing['url']).read())

    #Get the full text, the RSS feed doesn't have this for now
    body = text.format_contents(soup.find("div", {"id": "post_bodytext"}))
    #Next try for images
    images = []
    img_link_container = soup.find("div", {"id": "post_photos"})
    images = []
    if img_link_container:
...
def process_providerguide(rss_url, city, state):
        """
        Processes all listings for a specific providerguide RSS Feed
        """
        city_address = geolocation.get_best_guess_address(city=city, state=state)
        city_lat, city_lon = geolocation.decode_to_latlon(city_address)
